...
var fileWriteErrorMessage = function (filePath) {
    return "Could not write to `" + filePath + "`. Your configuration will not be persisted.";
};
...
var checkLockfile = function (filePath) {
    ...
    var errorMessage = "Config lockfile at " + filePath + " found. Are you running Lisk Commander in another process?";
    if (locked) {
        throw new Error(errorMessage);
    }
};
var attemptToReadJSONFile = function (filePath) {
    ...
    var errorMessage = "Config file cannot be read or is not valid JSON. Please check " + filePath + " or delete the file so we can create a new one from defaults.";
    return attemptCallWithError(fn, errorMessage);
};
var attemptToValidateConfig = function (config, filePath) {
    var rootKeys = constants_1.CONFIG_VARIABLES.map(function (key) { return key.split('.')[0]; });
    var fn = function () {
    ...
    var errorMessage = "Config file seems to be corrupted: missing required keys. Please check " + filePath + " or delete the file so we can create a new one from defaults.";
