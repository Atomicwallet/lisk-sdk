...
var validator = lisk_transactions_1.utils.validator, stringEndsWith = lisk_transactions_1.utils.stringEndsWith;
...
var DappTransaction = (function (_super) {
    ...
    DappTransaction.prototype.validateAsset = function () {
        ...
        if (this.type !== constants_1.TRANSACTION_DAPP_TYPE) {
            errors.push(new lisk_transactions_1.TransactionError('Invalid type', this.id, '.type', this.type, constants_1.TRANSACTION_DAPP_TYPE));
        }
        if (!this.amount.eq(0)) {
            errors.push(new lisk_transactions_1.TransactionError('Amount must be zero for vote transaction', this.id, '.amount', this.amount.toString(), '0'));
        }
        if (this.recipientId) {
        ...
        if (!this.fee.eq(constants_1.DAPP_FEE)) {
            errors.push(new lisk_transactions_1.TransactionError("Fee must be equal to " + constants_1.DAPP_FEE, this.id, '.fee', this.fee.toString(), constants_1.DAPP_FEE));
        }
