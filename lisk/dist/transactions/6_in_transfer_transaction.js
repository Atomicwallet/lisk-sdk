...
var convertBeddowsToLSK = lisk_transactions_1.utils.convertBeddowsToLSK, verifyAmountBalance = lisk_transactions_1.utils.verifyAmountBalance, validator = lisk_transactions_1.utils.validator;
...
var InTransferTransaction = (function (_super) {
    ...
    InTransferTransaction.prototype.prepare = function (store) {
        return tslib_1.__awaiter(this, void 0, void 0, function () {
            ...
            return tslib_1.__generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4, store.account.cache([{ address: this.senderId }])];
                    case 1:
                        _a.sent();
                    ...
                    case 2:
                        ...
                        dappTransaction = transactions && transactions.length > 0
                            ? transactions.find(function (tx) {
                                return tx.type === constants_1.TRANSACTION_DAPP_TYPE &&
                                    tx.id === _this.asset.inTransfer.dappId;
                            })
                            : undefined;
                        if (!dappTransaction) return [3, 4];
                        return [4, store.account.cache([
    ...
    InTransferTransaction.prototype.validateAsset = function () {
        ...
        var errors = lisk_transactions_1.convertToAssetError(this.id, validator.errors);
        if (this.type !== constants_1.TRANSACTION_INTRANSFER_TYPE) {
            errors.push(new lisk_transactions_1.TransactionError('Invalid type', this.id, '.type', this.type, constants_1.TRANSACTION_INTRANSFER_TYPE));
        }
        if (this.recipientId) {
        ...
        if (!this.fee.eq(constants_1.IN_TRANSFER_FEE)) {
            errors.push(new lisk_transactions_1.TransactionError("Fee must be equal to " + constants_1.IN_TRANSFER_FEE, this.id, '.fee', this.fee.toString(), constants_1.IN_TRANSFER_FEE));
        }
        return errors;
    };
    InTransferTransaction.prototype.applyAsset = function (store) {
        ...
        var idExists = store.transaction.find(function (transaction) {
            return transaction.type === constants_1.TRANSACTION_DAPP_TYPE &&
        ...
        var balanceError = verifyAmountBalance(this.id, sender, this.amount, this.fee);
        if (balanceError) {
            errors.push(balanceError);
        }
        var updatedBalance = new bignum_1.default(sender.balance).sub(this.amount);
        var updatedSender = tslib_1.__assign({}, sender, { balance: updatedBalance.toString() });
        ...
        var updatedRecipientBalance = new bignum_1.default(recipient.balance).add(this.amount);
        var updatedRecipient = tslib_1.__assign({}, recipient, { balance: updatedRecipientBalance.toString() });
    ...
    InTransferTransaction.prototype.undoAsset = function (store) {
        ...
        var dappTransaction = store.transaction.get(this.asset.inTransfer.dappId);
        var recipient = store.account.get(dappTransaction.senderId);
        var updatedRecipientBalance = new bignum_1.default(recipient.balance).sub(this.amount);
