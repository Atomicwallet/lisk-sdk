...
var verifyAmountBalance = lisk_transactions_1.utils.verifyAmountBalance, validator = lisk_transactions_1.utils.validator;
...
var OutTransferTransaction = (function (_super) {
    ...
    OutTransferTransaction.prototype.assetToBytes = function () {
        var _a = this.asset.outTransfer, dappId = _a.dappId, transactionId = _a.transactionId;
    ...
    OutTransferTransaction.prototype.verifyAgainstTransactions = function (transactions) {
        var _this = this;
    ...
    OutTransferTransaction.prototype.validateAsset = function () {
        ...
        if (this.type !== TRANSACTION_OUTTRANSFER_TYPE) {
            errors.push(new lisk_transactions_1.TransactionError('Invalid type', this.id, '.type', this.type, TRANSACTION_OUTTRANSFER_TYPE));
        }
        ...
        if (!this.fee.eq(constants_1.OUT_TRANSFER_FEE)) {
            errors.push(new lisk_transactions_1.TransactionError("Fee must be equal to " + constants_1.OUT_TRANSFER_FEE, this.id, '.fee', this.fee.toString(), constants_1.OUT_TRANSFER_FEE));
        }
        if (this.recipientId === '') {
    ...
    OutTransferTransaction.prototype.applyAsset = function (store) {
        var errors = [];
        var dappRegistrationTransaction = store.transaction.get(this.asset.outTransfer.dappId);
